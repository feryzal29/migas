...
class ClientWebController extends Controller
{
    public function index()
    {
        ...
        $posting2 = Posting::orderBy('id', 'DESC')->Limit(2)->get()
            ->each(function ($item) {
                $item->isi = Utility::cuplikan_isi($item->isi, 200);
            });
        $faq = faq::all();
        ...
        $works = Works::all();
        return view('index', compact(['header', 'profil', 'visi', 'myteams', 'videos', 'posting','posting','works', 'faq', 'contact']));
    }

    public function template()
    {
        return view('layout.templateWeb');
    }
